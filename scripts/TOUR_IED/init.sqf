...
if (!isDedicated) then
{
	if (isNil "TOUR_IED_scriptran") then
	{
		...
		<br />
		 ...
		 <br />
		 <br />
		  - Once close to an IED, you will have an action to examine it, and then defuse it. When ready, click the action and follow the instructions in the dialog. Defusal of an IED can be done by working out a sequence of 8 arrows. If the sequence is wrong, you will have to start again, and remember the progress you had made. Aborting the defusal process (Esc), or running out of time, will likely result in detonation of the IED.
		  <br />
		  <br />
