...
_pos2 = [objnull, [0,0,0]];
_pos2 = (_posarray call BIS_fnc_selectRandom);
_posarray = _posarray - [_pos2];
...
_dir = ((getPosATL (_pos1 select 0)) getDir (_pos1 select 1));
_unit = _grp createUnit [_type, _pos1 select 1, [], _dir, "NONE"];
waitUntil {!isNull _unit};
_unit setPos (_pos1 select 1);
_unit setUnitPos "UP";
_unit call Tour_fnc_garbageEH;
_unit call TOUR_fnc_skillAI;
sleep 1;
_wp = _grp addWaypoint [_pos2 select 1, 0];
...
_array pushBack [_grp, [_pos1 select 1, _pos2 select 1], "soldier", time];
